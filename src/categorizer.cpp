...
int main(int argc, char** args)
{
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        unsigned int height_digit_b = digit_e + 2;
        f.seekg(height_digit_b);
        do {
            f.read(&sym, 1);
        } while (sym != 0x0A);
        unsigned int height_digit_e = static_cast<unsigned int>(f.tellg()) - 2;
        c = 1;

        for(j = height_digit_e; j >= height_digit_b; j--, c *= 10)
        {
            f.seekg(j);
            f.read(&sym, 1);
